...
interface DatabasePromptRow {
  ...
  status: string | null;
  is_favorite: boolean;
  source_portal: PromptRecord["sourcePortal"] | null;
  ...
  const { error: promptError } = await client.from("prompts").upsert(
    {
      ...
      source_portal: prompt.sourcePortal || null,
      source_label: prompt.sourceLabel || null,
      source_context: prompt.sourceContext ?? null,
      prompt_sections: prompt.promptSections ?? null,
    },
    { onConflict: "id" },
  );
