...
4. Every operation must reference valid source beat IDs and include an explanation.
   Source beat IDs are deterministic opaque tokens and must not encode page labels, which may be missing, duplicated, or non-sequential in user-authored text.
