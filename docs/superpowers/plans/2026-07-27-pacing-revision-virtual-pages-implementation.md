...
**Pass 4 transactional Undo correction:** PASS — normal Undo now invokes one owner-scoped `undo_writer_pacing_revision_apply` transaction instead of sequencing live mutations in the client. The RPC locks the applied set/snapshot, issue, child rows, and live page/outline scope; validates exact applied changes, prior values, created-page range and layers, contiguous page set, current applied candidates, and latest applied outline; then atomically restores existing content, deletes exact created rows/outline, and reopens children plus the set. Exceptions after any earlier restore roll the entire RPC back. Lost responses reconcile from persisted status without client writes: `ready`/`partially_ready` is success and `applied` is unchanged/retryable. Edge Beats regeneration was also explicitly regression-bound to the existing upsert trigger that stales dependent Dialogue and removes completed progress. Focused coverage passes 5 files / 63 tests; the full relevant Pacing/persistence gate passes 19 files / 184 tests; the full project gate passes 141 files / 977 tests; the production build passes; full lint reports 0 errors and 71 existing repository warnings. Local migration execution was unavailable because the Docker daemon was stopped and no Supabase connector was exposed in this session; the migration remains undeployed.
