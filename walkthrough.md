...
- **Comic surface**: Multi-page (Webtoon / 2-page spread), 800x1200 logical canvas, BSP-style panel splitting with fixed gutter
...
- **Sub-Selection Prep (Phase 11 start)**: `ComicPanel.tsx` refactored with `contentMode` prop. When active, the Konva Transformer attaches to the internal image (cyan handles, rotation enabled) instead of the panel group (gold handles), enabling independent content manipulation within the clipped frame.
...
- Sub-Selection UI: wire `contentMode` toggle into ObjectToolbar or keyboard shortcut.
- Full sub-selection: complete independent image drag/scale/rotate persistence within panels.
- Snap guides in Cyber Cyan, configurable BSP gutter, page background color/image, and image objects outside panels (e.g. SFX/overlays).
