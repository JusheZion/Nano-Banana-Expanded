...
- **Character Studio UI:** `src/portals/CharacterStudio.tsx` — Left panel: Art Style Engine (flagship + library chips, Custom Style input + Save as Tag), DNA Engine (Heritage, Gender chips), Surgical Physical (multi-select chips per category), Wardrobe Engine (9 sub-categories with presets + input + Save to Library), Cinematic Suite (Angle, Shot, Lighting, Tone, Location), Onyx Vault (password to editable textarea), Prompt Tags (HybridTagBar). Right panel: Live Image area, Age modifier slider, Add Character Pose, Import Image, Live Prompt (compiled + Copy), footer: Generate Character, Save New Character, Save New Pose, Save Edited Pose, Cast in Story (disabled when no saved stories).
...
- **Import & spatial lock:** "Import Asset/Setting" upload zone at top of left panel. When an image is uploaded, Setting and Location sections (Era/Style, Location Type, Architectural Detail, Set Dressing & Props) are disabled and faded; "Diversify Style" checkbox re-enables them (composition/layout from image, era and materials from tags).
- **Left panel:** Art Style (flagship "Smooth Animated 3D/CGI Render" + library + Custom Style + Save as Tag), Era/Style, Location Type, Architectural Detail, Set Dressing & Props (Furniture, Lighting Fixtures, Surface Textures, Specific Prop Descriptions — each with input + Save to Library), Cinematic Suite (Angle: Low/High/Wide-angle/Macro; Tone: Melancholic/Heroic/Suspenseful/Gritty), Onyx Vault, Prompt Tags.
- **Right panel — Spatial Expansion Gallery:** Room Expansion (Interior from Exterior, Adjacent Room), Urban Expansion (Adjacent City Block, Aerial View), Time/Season (Night, Noon, Golden Hour, Winter (Snow), Autumn). Selections feed into Live Prompt.
- **Footer pills:** Generate Asset | Save New Asset | Expand Setting | Add to Library | Cast in Story. Cast in Story uses `storyPhotoCollections` (same as Character Studio); Save New Asset / Add to Library call `saveGeneration('asset', url)`.
- **Files:** `src/shared/theme/Phase12DesignTokens.ts` (ASSET_STUDIO_BG, ASSET_STUDIO_AMETHYST_TEXT), `src/data/asset_studio_spec.ts`, `src/stores/assetStudioStore.ts`, `src/shared/utils/assetStudioPrompt.ts`, `src/portals/AssetsStudio.tsx`.
