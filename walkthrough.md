...
- **Half-Circle, Quarter-Circle, Sector:** New `shapeType`s with path helpers in `circularPanelPaths.ts` (half: M -r 0 A r r 0 0 0 r 0 Z; quarter: M 0 0 L r 0 A r r 0 0 1 0 r Z; sector: M 0 0 L r 0 A r r 0 [largeArc] 1 [endX] [endY] Z). ObjectToolbar shape buttons; ComicPanel renders via Konva Path (fill, shadow, glow, stroke, clip). Sector has `centralAngle` (1–360°).
...
- **Hex and numeric inputs:** ColorWheelPicker hex field accepts typing (with or without #); applies when value is valid 3/6-digit hex or on blur/Enter. GradientBuilder: angle has a number input (0–360°) alongside the slider; per-stop color has a typable hex input (StopHexInput) with apply on valid/blur/Enter.
- **Undo/Redo (zundo + persist):** Undo worked once then stopped; redo never worked because zundo’s undo/redo were calling the wrapped `set`, which ran `temporalHandleSet` and corrupted history. **Patch in `node_modules/zundo/dist/index.js`:** undo/redo now use the store’s **original setState** (saved before the temporal wrapper is installed) so they apply state without pushing to pastStates/clearing futureStates. If you upgrade the `zundo` package, re-apply this patch or check whether upstream fixed it. Undo/Redo remain only on the **Edit** ribbon (removed from View).
