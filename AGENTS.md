...
- Archived Pacing Revision Sets are recoverable read-only history, not deleted or discarded records. Archive only `ready`, `partially_ready`, `applied`, or `failed` sets through the authenticated owner-scoped archive transaction with exact expected status and `updated_at`; atomically preserve the previous status and archive timestamp, order history by that timestamp, exclude archived sets from active queries, and never mutate official pages, outlines, Revision Items, or Child Changes while archiving. Batch Pacing Review must preflight every selected issue before its first AI call, use one confirmation for all unfinished eligible sets, skip applying or actively generating sets with a visible per-issue reason, and archive each issue's exact prior set only after that issue's new review succeeds. A review, archive, refresh, or preflight failure for one issue must preserve its prior set where applicable, remain visible and actionable, and never cancel later eligible issues. Batch summaries must count a successfully saved review even when its archive or post-review refresh needs attention, report saved and attention/skipped totals separately, and expose progress plus attention through accessible live status and alert surfaces in both Simple and Advanced Story Review. Database triggers must reject every direct `INSERT`, `UPDATE`, or `DELETE` that would create or mutate an archived set, its Revision Items, or its Child Changes, including service-role writes; only the guarded archive transaction may perform the exact terminal-to-archived set transition. Client and Edge Function mutation paths must also reject archived records before generation or persistence and must verify every affected parent/item/set write before reporting success. Page-preview generation must acquire an owner-scoped parent lease by exact editable status and `updated_at` before AI or persistence, hold the set in archive-ineligible `generating` state through every Child Change and Revision Item write, and publish the candidate Child Change, Item generation state, parent progress/failure state, final status, and lease release in one owner-scoped `SECURITY DEFINER` transaction that locks and revalidates the exact unexpired lease before its first write. The lease duration must exceed two supported 75-second Gemini attempts plus prompt and persistence overhead; it is currently four minutes, remains committable through its exact expiry instant, and becomes recoverable only after expiry. A concurrent archive or second preview must lose before mutation; after an expired lease is recovered and replaced, the stale request must affect zero rows. Expired leases may return to their prior editable state but require a fresh retry. Ordinary Item/Change mutations are forbidden while `generating` unless the exact transaction-local write marker belongs to the current lease. Every direct Revision Item or Child Change mutation must lock and advance the parent set’s aggregate `updated_at` so previously observed archive guards become stale, and every lease metadata/status transition must be guarded against direct owner or service-role writes.
- Pacing Revision expansion pages must be fully previewed as virtual pages before Apply. The saved deterministic `proposed_outline_json` is authoritative: a virtual page is authorized only when it lies in the deterministically derived affected-page ownership of a persisted Revision Item with at least one accepted Outline operation, and affected-page ownership must compare deterministic source and proposed assignments so moves, combines, and insertions own every actually changed page rather than model-declared page claims. Compute ownership for each original item first, then merge items only when those deterministic page sets overlap; never merge from model-declared affected pages. Virtual children use `page_id: null` plus `virtual-page:<number>` and must never create or mutate `writer_pages` during generation or review. Virtual Page Beats depend on the owning item's applicable accepted Outline changes, including operations whose result page begins a shifted range before the virtual target; virtual Dialogue requires the effective dependent Beats candidate. Regenerated Beats must upsert the same child row so the database trigger atomically marks dependent Dialogue `stale`/`pending`, removes that page from completed progress, and queues Dialogue regeneration. Apply may create a virtual page only when its Outline, Beats, and Dialogue are complete and approved. Reload the authoritative Revision Set, issue lock notes, outline, and physical pages before preflight, and require physical page numbers to be contiguous. Preassign the outline and every possible page UUID, persist all cleanup identities in the guarded `applying` snapshot before the first insert, and use those caller IDs for inserts so interruption recovery can delete exact rows even when no insert response returned. Apply must map new live IDs, read back the target count and content, and atomically transition the set and exact child changes from `applying` to `applied` through an authenticated owner-scoped database transaction that locks and revalidates the expected latest outline, the exact contiguous page-number set `1..target`, page identities, and approved candidate values immediately before status mutation. An applied virtual child retains immutable `page_id: null` proposal provenance, but its persisted `page_number` must navigate and render as the created physical page once `generation_status` is `applied`. If the completion response is ambiguous, reload the set before cleanup: a persisted `applied` state is success only after fresh content verification, a confirmed `applying` state may compensate, and unreadable or other states must not mutate live data. Before destructive Apply recovery, strictly parse the persisted snapshot and bind its change, prior-page, created-page, issue, and outline identities to freshly loaded authoritative records. Apply/cleanup page reads and mutations must return explicit errors and exact affected rows rather than collapsing read errors or zero-row writes to success. After an aborted Apply, verified restoration plus exact created-page and planned-outline absence returns the set to retryable `ready`; incomplete cleanup remains `applying` with `recovery_required` detail. Normal Undo must use one authenticated owner-scoped database transaction that locks the authoritative applied set and stored snapshot plus the issue's pages and outlines; validates exact change, prior-page, created-page, contiguous page-set, applied-candidate, and latest applied-outline identity/content; restores existing Beats/Dialogue; deletes exact created pages and the applied outline; and reopens exact applied children plus the set to `ready` or `partially_ready`. Any failure rolls back every Undo write, including an error after an earlier restore. The client must not perform live Undo mutations. If the RPC response is ambiguous, reload only the set: `ready` or `partially_ready` is committed success, `applied` means live content is unchanged and Undo remains retryable, and unreadable/unknown state must fail visibly without further mutation. A pre-transaction legacy applied snapshot may be normalized only after the strict Undo rejects it as invalid: an authenticated owner-scoped adapter must accept only the exact legacy field boundary, derive the current contiguous physical page count and exact latest applied-outline identity/JSON, perform no live page or outline mutation itself, and delegate restoration to the same strict transactional Undo. Modern, partially modern, or otherwise corrupted snapshots remain ineligible and fail closed.
- Pacing Revision selection and batch decisions must be scoped to the active layer tab. Provide select-all and clear-selection controls for that tab, keep batch approve/reject controls visible while Revision Items scroll, and never include hidden selections from another layer. Layer `remaining` counts include only pending changes whose generation status is `ready`; failed, stale, locked, and not-yet-generated rows are unavailable rather than remaining decisions. Collapse large failed-layer ledgers by default, keep retry actions discoverable, and state that Page Beats or Dialogue generation failures do not block independent Outline approval.
