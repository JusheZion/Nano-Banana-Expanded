...
- Archived Pacing Revision Sets are recoverable read-only history, not deleted or discarded records. Archive only `ready`, `partially_ready`, `applied`, or `failed` sets through the authenticated owner-scoped archive transaction with exact expected status and `updated_at`; exclude archived sets from active queries, and never mutate official pages, outlines, Revision Items, or Child Changes while archiving.
