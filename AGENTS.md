...
- Writer AI Treatment modes must be preview-first, preserve source-story traceability, and obey the approved deterministic contracts in `docs/superpowers/specs/2026-07-23-writer-ai-treatment-contracts-design.md`; prompt wording alone is not sufficient enforcement. Keep My Order changes language and formatting only, Organize and Polish permits traceable structural changes within 10%, and Expand Creatively permits traceable enhancements/additions within 20%. Gemini must return one compact treated-beat list; the Edge Function derives the manifest locally so proposal and manifest content are not duplicated across the model response. Every proposal beat must map one-to-one with a unique manifest result, every source beat must be consumed exactly once, combined results must replace rather than coexist with their sources, and the declared page count must equal the actual proposal beat count. A schema-valid but cross-field-inconsistent model response may receive one diagnostic repair attempt, after which the same strict validation must pass or return actionable consistency details. The client must preserve the source outline's title, premise, acts, and notes when applying treated page beats. Production QA for this workflow must use a representative long outline near the supported 70-page case and verify completion below the hosted Edge limit, not only a smaller fixture. Promotion or alternate retention must preserve the manifest and official versions must remain recoverable.
