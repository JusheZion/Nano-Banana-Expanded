...
- Writer AI Treatment modes must be preview-first, preserve source-story traceability, and obey the approved deterministic contracts in `docs/superpowers/specs/2026-07-23-writer-ai-treatment-contracts-design.md`; prompt wording alone is not sufficient enforcement. Keep My Order changes language and formatting only, Organize and Polish permits traceable structural changes within 10%, and Expand Creatively permits traceable enhancements/additions within 20%. A schema-valid but cross-field-inconsistent model response may receive one diagnostic repair attempt, after which the same strict validation must pass or return actionable consistency details. Promotion or alternate retention must preserve the manifest and official versions must remain recoverable.
