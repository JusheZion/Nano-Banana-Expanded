...
- Writer AI Treatment modes must be preview-first, preserve source-story traceability, and obey the approved deterministic contracts in `docs/superpowers/specs/2026-07-23-writer-ai-treatment-contracts-design.md`; prompt wording alone is not sufficient enforcement. Keep My Order changes language and formatting only, Organize and Polish permits traceable structural changes within 10%, and Expand Creatively permits traceable enhancements/additions within 20%. Every proposal beat must map one-to-one with a unique manifest result, every source beat must be consumed exactly once, combined results must replace rather than coexist with their sources, and the declared page count must equal the actual proposal beat count. A schema-valid but cross-field-inconsistent model response may receive one diagnostic repair attempt, after which the same strict validation must pass or return actionable consistency details. Promotion or alternate retention must preserve the manifest and official versions must remain recoverable.
