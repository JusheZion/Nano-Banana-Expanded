...
- [x] Simplify Writers Workshop chronology to `Library -> Foundation -> Synopsis -> Canon -> Outline -> Pages -> Beats -> Dialogue -> Visual Prep -> Audit -> Cockpit -> Export` with a tested workflow helper and updated production map.
...
- [x] Deploy and verify visual-reference-aware `writer-tools`: Supabase project `vxclogwiytxjolisnakd` showed `writer-tools` ACTIVE version 48, and signed-in local browser QA generated full Page 1 beats using attached `Alpha Swag Aries` plus `Expanded View of Prime Hall` visual canon.
...
- [x] Visual-reference QA pass: focused Writer tests, build/lint/diff checks, Supabase deployed-function auth validation, signed-out smoke, and signed-in local in-app browser QA passed. The signed-in QA attached one Character Vault reference and one Asset Vault reference to a disposable Writer issue, confirmed persistence after reload, generated Page 1 beats through deployed `writer-tools`, and verified the beats used `Alpha Swag Aries` plus `Expanded View of Prime Hall` as visual canon. Temporary local QA issue/series cleanup is pending explicit deletion approval because it deletes real signed-in account data.
...
- [x] Rename portal `lab` in nav/headers/wiki framing: **Image Workshop** → **Illustrator’s Imageshop**
- [x] Rename Supporting Resources Vault label to **NPC Vault**
- [x] NPC Vault button parity: add the same add/use affordances as the other two vault lanes
- [x] Production panel density: tighten action icon spacing so controls sit next to the labels they affect
- [x] Tooltips for key panel headers + icon buttons (first-time guidance)
- [x] Beat Timeline: make **Delete beat** obvious (button + tooltip)
- [x] Image Lab: add landscape-friendly preview region for wide ratios (e.g. 21:9)
- [x] Beat Detail: add dropdown/select helpers for common choices while keeping free-text
- [x] Writer → Imageshop handoff: make outline/wiki-like entries either actionable or clearly reference-only
- [x] Camera angle workflow hint/preset: encourage “same image, POV-only change” without identity drift
...
## Asset Studio 2.0 — phone layout (Phase 5, 2026-04-14) — COMPLETE

- [x] Exclusive workspace panels on phone (no stacked refs+build+prompt); default mode **output**; workspace tabs on all breakpoints
- [x] Preview-first column order on mobile; build sub-tabs + Simple/Advanced on phone; 44px min touch targets on small screens
- [x] Live Prompt panel flex/height adjusted for phone
- [x] Verify: `npm run test -- --run`, `npm run build`
...
- [x] **Cloudflare Builds connected:** Cloudflare **Workers → Settings → Build** is connected to `JusheZion/Nano-Banana-Expanded`, production branch **`main`**, build command **`npm ci && npm run build`**, deploy command **`npx wrangler deploy --config ./wrangler.jsonc`**, root directory **`/`**. Cloudflare manages the build API token; do not duplicate this with a GitHub Actions deploy token unless intentionally switching pipelines.
- [ ] **Operator:** In Cloudflare **Workers** project **Settings → Build**, confirm the **Version command** is **`npm run cf:versions-upload`** or **`npx wrangler versions upload --config ./wrangler.jsonc`** if preview/branch builds are enabled later.
