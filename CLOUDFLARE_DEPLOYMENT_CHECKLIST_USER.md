...
- [x] **D4f. Workers Builds token:** If the Worker is connected to GitHub through Cloudflare **Settings → Build**, Cloudflare manages its own API token. Do **not** copy the Cloudflare **API token name** into GitHub Actions; it is not the token value. In this mode, deploys should be triggered and debugged from Cloudflare Builds, not a separate GitHub Actions deploy workflow.
